...
shared by every task via LocalExecutor), a single task then PUTs each
dataset's files into a Snowflake internal stage and COPY INTOs its raw
table, and dbt builds the analytics models on top. Task bodies live in
historical_race_batch.py so they can also be run standalone for local
backfills.
"""
...
def f1_historical_batch():
    ...
    def load_to_snowflake(_extracted: list[dict[str, str]]) -> None:
        load_all_datasets_to_snowflake()
